...
export default {
  ...
  computed: {
    ...
    product() {
      return this.findByProduct(this.productId, this.account)
    },
    ...
    editLink() {
      return this.localePath({ name: 'dashboard-products-id-edit', params: { id: this.product?.id } })
    },
  },
  ...
  methods: {
    ...
    async archive() {
      if (this.account) {
        await this.update({ documentId: this.product?.id, payload: { account: this.account, active: false } })
      }
    },

    async unarchive() {
      if (this.account) {
        await this.update({ documentId: this.product?.id, payload: { account: this.account, active: true } })
      }
    },
  },
}
